...
(defun caml-set-compile-command ()
  ...
  (unless (or (null buffer-file-name)
              ;; (file-exists-p "makefile")
              ;; (and (file-exists-p "Makefile") (not (file-exists-p "OMakefile")))
              (file-exists-p "_build")
              (file-exists-p "_tags"))
    (let* ((filename (file-name-nondirectory buffer-file-name))
           (basename (file-name-sans-extension filename))
           (command nil))
      (cond
       ((string-match ".*\\.mfi\$" filename)
        (setq command (concat "omake " basename ".cmo")))
       ((string-match ".*\\.mf\$" filename)
        (setq command (concat "omake " basename ".cmo")))
       ((string-match ".*\\.mli\$" filename)
        (setq command (concat "mlfic -c " filename)))
       ((string-match ".*\\.ml\$" filename)
        (setq command (concat "mlfic -c " filename)))
       ((string-match ".*\\.mll\$" filename)
        (setq command (concat "mlfilex " filename)))
       ((string-match ".*\\.mll\$" filename)
        (setq command (concat "mlfiyacc " filename)))
       )
      (if command
          (progn
            (make-local-variable 'compile-command)
...
(defun caml-find-alternate-file ()
  ...
  (let ((name (buffer-file-name)))
    (if (string-match "^\\(.*\\)\\.\\(ml\\|mli\\)$" name)
      ...
      (if (string-match "^\\(.*\\)\\.\\(mf\\|mfi\\)$" name)
          (find-file
           (concat
            (caml-match-string 1 name)
            (if (string= "mf" (caml-match-string 2 name)) ".mfi" ".mf")))
        )
      )
    ))
